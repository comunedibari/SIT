...
from usersmanage.utils import get_fields_by_user, crispyBoxACL, userHasGroups, get_viewers_for_object, \
    get_user_groups_for_object, AuthGroup, User, get_users_for_object
...
class cduConfigInitForm(G3WFormMixin, G3WRequestFormMixin, G3WACLForm, ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Fill users and groups user
        if not self.request.user.is_superuser:
            if kwargs['data'] and f"{kwargs['prefix']}-project" in kwargs['data'] or \
                    f"{kwargs['prefix']}-project" in kwargs['initial']:

                if f"{kwargs['prefix']}-project" in kwargs['data']:
                    project_pk = kwargs['data'][f"{kwargs['prefix']}-project"]
                else:
                    project_pk = kwargs['initial'][f"{kwargs['prefix']}-project"]

                project = Project.objects.get(pk=project_pk)
                editor_user = get_users_for_object(project, 'change_project', [G3W_EDITOR1],
                                                                           with_anonymous=False)
                self.fields['editor_user'].queryset = User.objects.filter(pk__in=[u.pk for u in editor_user])
                editor2_user = get_users_for_object(project, 'change_project', [G3W_EDITOR2],
                                                                           with_anonymous=False)
                self.fields['editor2_user'].queryset = User.objects.filter(pk__in=[u.pk for u in editor2_user])
                viewer_users = get_users_for_object(project, 'view_project', [G3W_VIEWER1, G3W_VIEWER2],
                                                    with_anonymous=True)
                self.fields['viewer_users'].queryset = User.objects.filter(pk__in=[u.pk for u in viewer_users])
                editor_user_groups = get_user_groups_for_object(project, self.request.user, 'change_project', 'editor')
                self.fields['editor_user_groups'].queryset = AuthGroup.objects.filter(
                    pk__in=[a.pk for a in editor_user_groups])
                viewer_user_groups = get_user_groups_for_object(project, self.request.user, 'view_project', 'viewer')
                self.fields['viewer_user_groups'].queryset = AuthGroup.objects.filter(
                    pk__in=[a.pk for a in viewer_user_groups])
