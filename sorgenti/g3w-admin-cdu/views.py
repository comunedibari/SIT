    ...
    def get_form_initial(self, step):
        if 'slug' in self.kwargs:
            if step == '0':
                ...
                # remove users that are editor1 and editor2 with 'change_configs' permission
                res['viewer_users'] = []
                for o in viewers:
                    if not o.has_perm(self.editor_permission[0], self.currentCduConfig):
                        res['viewer_users'].append(o.id)
...
class CduConfigDeleteView(G3WAjaxDeleteViewMixin, G3WRequestViewMixin, SingleObjectMixin, View):
    ...
    @method_decorator(permission_required('cdu.delete_configs', (Configs, 'slug', 'slug'), return_403=True))
    def dispatch(self, *args, **kwargs):
...
class CduCreatedocView(View):
    ...
    def post(self, request, **kwargs):
        ...
        odt = ODT.factory(config, results, result_ids_to_show=selected_results_ids, map_image=map_image,
                          output_format=output_format, request=request)
...
class UsersGroupsConfigView(View):
    ...
    def get(self, *args, **kwargs):
        ...
        if cdu_config:
            ...
            # Only strict viewer users remove user editor width editor_permission_configs
            viewers = []
            for o in viewer_users_selected:
                if not o.has_perm(self.editor_permission_configs, cdu_config):
                    viewers.append(o)
            viewer_users_selected = viewers
...
class TDDwhAuditApiView(G3WAPIView):
    """
