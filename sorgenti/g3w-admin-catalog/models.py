...
class Catalog(models.Model):
    ...
    inspire_date = models.DateField(
        _('INSPIRE date'), blank=True, null=True,
    ...
    inspire_contact_organization = models.CharField(
        _('Contact point: organization'), max_length=255, blank=True, null=True,
...
class EULicense(models.Model):
    """
    Model to store RDF EU license library: http://publications.europa.eu/resource/authority/licence
    """

    op_code = models.CharField(_('OP code'), max_length=50)
    definition = models.TextField(_('Definition'))
    url = models.URLField(_('License URL'))

    def __str__(self):
        return self.op_code


class InspireLimitationsOnPublicAccess(models.Model):
    """
    Model to store RDF EU license library: https://inspire.ec.europa.eu/metadata-codelist/LimitationsOnPublicAccess
    """

    inspire_id = models.CharField(_('OP code'), max_length=400)
    label = models.CharField(_('label'), max_length=400)
    definition = models.TextField(_('Definition'))
    url = models.URLField(_('Limitation URL'))
...
class Record(models.Model):
    ...
    - 2.4.1 Limitazione d’uso (No conditions apply)
    - 2.4.2 Vincoli di accesso (Altri vincoli - Dato pubblico)
    - 2.4.3 Vincoli di fruibilità
    - 2.4.3 Altri vincoli (Altri vincoli - Dato pubblico)
    ...
    - 2.3.6 Parole chiave - !! GEMET only is allowed !!
    ...
    catalog = models.ForeignKey(
        Catalog, on_delete=models.CASCADE, help_text=_('Maps to pycsw:ParentIdentifier'))
    ...
    identifier = models.CharField(
        ...
        max_length=255,
        db_index=True,
        help_text=_("File identifier - Maps to pycsw:Identifier")
    )
    ...
    metadata_source = models.CharField(
        ...
        max_length=255,
        default="local",
        help_text=_('maps to pycsw:MdSource'),
        db_index=True
    )
    # FIXME: check if can be removed
    insert_date = models.DateTimeField(
        editable=False,
        auto_now_add=True,
        help_text=_('Maps to pycsw:InsertDate')
    )
    ...
    xml = models.TextField(
        ...
        help_text=_('Maps to pycsw:XML'),
        editable=False,
    )
    # FIXME: check if can be removed
    any_text = models.TextField(
        help_text=_('Maps to pycsw:AnyText'),
        editable=False,
    )
    ...
    keywords = MultiSelectField(
        _('2.3.6 Keywords'), choices=Catalog.INSPIRE_GEMET_THEMES, max_length=1024, blank=True, null=False,
        help_text="{} | {}".format(
    ...
    keywords_types = models.CharField(
        ...
        blank=True,
        null=True,
        help_text=_('Maps to pycsw:Keywordstype')
    )
    format = models.CharField(
        ...
        help_text=_("Description about data format")
    )
    source = models.CharField(
        ...
        blank=True,
        null=True,
        help_text=_('Maps to pycsw:Source')
    )
    # FIXME: this is apparently not used, dateStamp is taken from the parent Catalog value
    date = models.DateTimeField(
        null=True, blank=True,
        # auto_now_add=True,
        help_text=_('Maps to pycsw:Date')
    )
    modified_date = models.DateField(
        _('Modified date'),
        null=True, blank=True,
        # auto_now_add=True,
        help_text="{} | {}".format(_('Modified date'), _('DCAT-US: <a href="https://resources.data.gov/resources/dcat-us/#modified">"<i>modified</i>"</a>'))
    )
    creation_date = models.DateField(
        _('Creation date'),
        null=True, blank=True,
        # auto_now_add=True,
        help_text="{} | {}".format(_('Data creation date'), _('DCAT-US: <a href="https://resources.data.gov/resources/dcat-us/#issued">"<i>issued</i>"</a>'))  # Maps to pycsw:CreationDate
    )
    revision_date = models.DateField(
        _('Revision date'),
        null=True, blank=True,
        # auto_now_add=True,
        help_text=_('Data revision date')  # Maps to pycsw:RevisionDate
    )
    publication_date = models.DateField(
        ...
        help_text=_('Data publication date')
    )
    # FIXME: check if can be removed
    type = models.CharField(
        ...
        null=True,
        blank=True,
        help_text=_('Maps to pycsw:Type')
    )
    bounding_box = models.TextField(
        ...
        help_text=_(
            'Data bounding box. Automatically calculated from layer')
    )
    crs = models.CharField(
        ...
        blank=True,
        # Maps to pycsw:CRS
        help_text=_('Data Coordinate reference system')
    )
    # FIXME: remove: unused
    alternate_title = models.CharField(
        ...
        blank=True,
        null=True,
        help_text=_('Maps to pycsw:AlternateTitle')
    )
    # FIXME: remove: unused
    organization_name = models.CharField(
        ...
        null=True,
        blank=True,
        help_text=_('Maps to pycsw:OrganizationName')
    )
    # FIXME: remove: unused
    security_constraints = models.CharField(
        _('Security constraints'),
        # editable=False,
        max_length=255,
        null=True,
        blank=True,
    ...
    topic_category = models.CharField(
        ...
        help_text=_('Main topic data')  # Maps to pycsw:TopicCategory
    )
    resource_language = models.CharField(
        ...
        help_text=_('Language used for data - inherited from catalog if left blank')
    )
    # FIXME: check if can be removed
    geographic_description_code = models.CharField(
        ...
        null=True,
        blank=True,
        help_text=_('Maps to pycsw:GeographicDescriptionCode')
    )
    denominator = models.IntegerField(
        _('Spatial resolution'),
        default=1000,
        help_text=_('General density data factor')
    )
    distance_value = models.CharField(
        ...
        null=True,
        blank=True,
        help_text=_('Spatial resolution - Maps to pycsw:DistanceValue')
    )
    ...
    service_type = models.CharField(
        ...
        null=True,
        blank=True,
        help_text=_('Maps to pycsw:ServiceType')
    )
    # FIXME: check if can be removed: hardcoded
    service_type_version = models.CharField(
        ...
        null=True,
        blank=True,
        help_text=_('Maps to pycsw:ServiceTypeVersion')
    )
    # FIXME: check if can be removed: hardcoded
    operation = models.CharField(
        ...
        null=True,
        blank=True,
        help_text=_('Maps to pycsw:Operation')
    )
    # FIXME: check if can be removed: hardcoded
    coupling_type = models.CharField(
        ...
        null=True,
        blank=True,
        help_text=_('Maps to pycsw:CouplingType')
    )
    # FIXME: check if can be removed: hardcoded
    operates_on = models.CharField(
        ...
        null=True,
        blank=True,
        help_text=_('Maps to pycsw:OperatesOn')
    )
    # FIXME: check if can be removed: hardcoded
    operates_on_identifier = models.CharField(
        ...
        null=True,
        blank=True,
        help_text=_('Maps to pycsw:OperatesOnIdentifier')
    )
    # FIXME: check if can be removed: hardcoded
    operates_on_name = models.CharField(
        ...
        null=True,
        blank=True,
        help_text=_('Maps to pycsw:OperatesOnName')
    )
    # FIXME: check if can be removed: hardcoded
    degree = models.CharField(
        ...
        null=True,
        blank=True,
        help_text=_('Maps to pycsw:Degree')
    )
    # FIXME: check if can be removed: deprecated
    access_constraints = models.TextField(
        _('Access constraints'),
        # editable=False,
        null=True,
        blank=True,
    ...
    access_constraints_inspire = models.ForeignKey(
        InspireLimitationsOnPublicAccess,
        verbose_name=_('Access constraints'),
        null=True,
        blank=False,
        on_delete=models.DO_NOTHING,
        help_text=_('Select a access contraints from the list')
    ...
    usability_constraints = models.TextField(
        _('Usability constraints'),
        # editable=False,  # auto
        null=True,
        blank=True,
        help_text=_('Conditions applicable to access and use of spatial data sets and to '
                    'related services and, where applicable, to the corresponding fees, in accordance with the law '
                    'of article 5, paragraph 2, letter b), and of article 11, paragraph 2, letter '
    ...
    classification = models.CharField(
        ...
        null=True,
        blank=True,
        help_text=_('Maps to pycsw:Classification')
    )
    # FIXME: check if can be removed: hardcoded
    condition_applying_to_access_and_use = models.TextField(
        ...
        null=True,
        blank=True,
        help_text=_('Maps to pycsw:ConditionApplyingToAccessAndUse')
    )
    lineage = models.TextField(
        ...
        help_text=_(
            'Descriptive text about process history and/or general quality of data')
    )
    responsible_party_role = models.CharField(
        ...
        null=True,
        blank=True,
        help_text=_('Maps to pycsw:ResponsiblePartyRole')
    )
    specification_title = models.CharField(
        ...
        null=True,
        blank=True,
        help_text=_('Maps to pycsw:SpecificationTitle')
    )
    specification_date = models.DateTimeField(
        editable=False,  # hardcoded
        blank=True,
        null=True,
        help_text=_('Maps to pycsw:SpecificationDate')
    )
    specification_date_type = models.CharField(
        ...
        null=True,
        blank=True,
        help_text=_('Maps to pycsw:SpecificationDateType')
    )
    creator = models.CharField(
        ...
        null=True,
        blank=True,
        help_text=_('Maps to pycsw:Creator')
    )
    publisher = models.CharField(
        ...
        null=True,
        blank=True,
        help_text=_('Maps to pycsw:Publisher')
    )
    contributor = models.CharField(
        ...
        null=True,
        blank=True,
        help_text=_('Maps to pycsw:Contributor')
    )
    relation = models.CharField(
        ...
        null=True,
        blank=True,
        help_text=_('Maps to pycsw:Relation')
    )
    links = models.CharField(
        ...
        null=True,
        blank=True,
        help_text=_('Maps to pycsw:Links')
    )
    accuracy = models.FloatField(
        _('Positional accuracy'),
        default=1.0,
        help_text=_('Positional accuracy in meters')
    )

    # INSPIRE overrides

    inspire_use_limitation = models.TextField(
        _('Use limitation'), blank=True, null=True,
        help_text=_('Use limitation - Restrictions on data usage and access, inherited from catalog - No conditions apply - if left blank'))

    inspire_contact_organization = models.CharField(
        _('INSPIRE contactor organization'), max_length=255, blank=True, null=True,
        help_text="{} | {}".format(
            _('Responsabile dei metadati - the organization name responsible for the INSPIRE/RNDT metadata, inherited from catalog if left blank'),
            _('DCAT-US: <a href="https://resources.data.gov/resources/dcat-us/#publisher">"<i>publisher</i>"</a>')
            )
        )
    inspire_contact_email = models.CharField(
        _('INSPIRE contact email'), max_length=255, blank=True, null=True,
        help_text=_('Responsabile dei metadati - the email address of entity responsible for the INSPIRE/RNDT metadata, inherited from catalog if left blank'))
    inspire_contact_url = models.CharField(
        _('INSPIRE contact url'), max_length=255, blank=True, null=True,
        help_text=_('Responsabile dei metadati - the URL of entity responsible for the INSPIRE/RNDT metadata, inherited from catalog if left blank'))
    inspire_contact_phone = models.CharField(
        _('INSPIRE contact phone'), max_length=255, blank=True, null=True,
        help_text=_('Responsabile dei metadati - the telephone number of entity responsible for the INSPIRE/RNDT metadata, inherited from catalog if left blank'))

    # RNDT Distributor overrides
    rndt_distributor_contact_organization = models.CharField(
        _('Data distributor: organization'),
        max_length=255,
        blank=True, null=True,
        help_text=_('Data distributor - RNDT the organization name of the distributor, inherited from catalog if left blank'))
    rndt_distributor_contact_email = models.CharField(
        _('Data distributor: email'),
        max_length=255,
        blank=True, null=True,
        help_text=_('Data distributor - the email address of the distributor, inherited from catalog if left blank'))
    rndt_distributor_contact_url = models.CharField(
        _('Data distributor: URL'),
        max_length=255,
        blank=True, null=True,
        help_text=_('Data distributor - the URL of the distributor, inherited from catalog if left blank'))
    rndt_distributor_contact_phone = models.CharField(
        _('Data distributor: phone'),
        max_length=255,
        blank=True, null=True,
        help_text=_('Data distributor - the telephone number of the distributor, inherited from catalog if left blank'))

    # INSPIRE Owner overrides (citedResponsibleParty)
    inspire_owner_contact_organization = models.CharField(
        _('INSPIRE owner contact organization'), max_length=255, blank=True, null=True,
        help_text=_('Responsabile - INSPIRE the organization name of the owner, inherited from catalog if left blank'))
    inspire_owner_contact_email = models.CharField(
        _('INSPIRE owner contact email'), max_length=255, blank=True, null=True,
        help_text=_('Responsabile - the email address of the owner, inherited from catalog if left blank'))
    inspire_owner_contact_url = models.CharField(
        _('INSPIRE owner contact url'), max_length=255, blank=True, null=True,
        help_text=_('Responsabile - the URL of the owner, inherited from catalog if left blank'))
    inspire_owner_contact_phone = models.CharField(
        _('INSPIRE owner contact phone'), max_length=255, blank=True, null=True,
        help_text=_('Responsabile - the telephone number of the owner, inherited from catalog if left blank'))
    ...
    rndt_resource_maintenance_frequency = models.CharField(
        _('Frequenza di aggiornamento'),
        max_length=255,
        default='unknown',
        choices=INPIRE_RESOURCE_MAINTENANCE_FREQUENCY,
        help_text=_(
            'Frequenza con la quale sono registrati gli aggiornamenti dei dati.')
    )
    ...
    # EU License
    eu_license = models.ForeignKey(EULicense,
                                   verbose_name=_('Usability constraints - data license'),
                                   null=True,
                                   blank=True,
                                   on_delete=models.DO_NOTHING,
                                   help_text='{} | {}'.format(_('Conditions applicable to access and use of spatial data sets and to '
                    'related services and, where applicable, to the corresponding fees, in accordance with the law '
                    'of article 5, paragraph 2, letter b), and of article 11, paragraph 2, letter '
                    'f), of Directive 2007/2 / EC. '),
                              _('<a href="/catalog/eulicense"><i>Licenses list</i></a>')))
