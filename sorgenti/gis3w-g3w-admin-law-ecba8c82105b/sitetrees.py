...
sitetrees = (
  # Define a tree with `tree` function.
    G3Wtree('law', title='Laws', module='law', items=[
      ...
      item('Normative', 'law-list', icon_css_class='fa fa-legal', children=[
          item('Aggiungi normativa', 'law-add', url_as_pattern=True, icon_css_class='fa fa-plus',
               access_by_perms=['law.add_laws']),
          item('Lista normative', 'law-list', url_as_pattern=True, icon_css_class='fa fa-list'),
          item('Lista articoli {{ law.slug }}', 'law-article-list law.slug', in_menu=False, url_as_pattern=True,
               icon_css_class='fa fa-list',
               children=[
                item('Aggiungi articolo {{ law.slug }}', 'law-article-add law.slug', in_menu=False, url_as_pattern=True,
                     icon_css_class='fa fa-plus'),
                item('Agg. articolo {{ law.slug }} {{ object.slug }}', 'law-article-update law.slug object.slug',
    ...
    G3Wtree('law_en', title='Laws', module='law', items=[
        # Then define items and their children with `item` function.
        item('LAW', '#', type_header=True),
        item('Laws', 'law-list', icon_css_class='fa fa-legal', children=[
