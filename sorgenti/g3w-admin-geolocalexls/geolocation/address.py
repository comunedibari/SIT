    ...
    def geolocate(self):
        ...
        for dt in self.ds:
            try:
                ...
                # For numcv
                try:
                    numciv = int(dt[self.ds.headers.index('numciv')])
                except Exception as e:
                    logger.error(f'Numciv error on row {dt}: {e}')
                    numciv = ''
                ...
                try:
                    ...
                    logger.error(f'Postalcode error on row {dt}: {e}')
                    postalcode = ''

                query = {
                  'street': f"{dt[self.ds.headers.index('indirizzo')]} {numciv}",
