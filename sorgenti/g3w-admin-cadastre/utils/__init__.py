...
def get_data_for_widget_search(data_project):
    ...
    # For every layers by config
    # --------------------------

    layer_ids = []
    for clayer in data_project.configlayer_set.all():

        if clayer.layer.origname == cadastre_layer_origname:

            layer_id = clayer.layer.qgs_layer_id
            ...
            layer_ids.append((layer_id, queryurl, clayer.layer.title))
