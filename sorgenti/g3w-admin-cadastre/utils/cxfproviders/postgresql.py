...
class PGProvider(BaseProvider):
    ...
    def clear_table_data_by_cod_com(self):
        ...
        """
        logger.info("Clearing data table")
        logger.info(f"DELETE FROM {self.schema}.{self.tablename} WHERE codice_comune='{self.cod_com}'")
    ...
    def check_crs(self):
        ...
        cursor = self.connection.cursor()
        cursor.execute(f"SELECT Find_SRID('{self.schema}','{self.tablename}','the_geom')")
    ...
    def create_table(self):
        ...
        sql = f"CREATE TABLE {self.schema}.{self.tablename} " \
              ...
              f"the_geom geometry(Polygon,{self.srid})," \
        ...
        sql = f"CREATE INDEX sidx_{self.schema}_{self.tablename}_the_geom " \
              f"ON {self.schema}.{self.tablename} " \
              f"USING gist (the_geom)"
