...
class Fornitura(object):
    ...
    def _readCSVFile(self, zipfile, csvFile):
        ...
        reader = csv.reader((l.decode().replace('\0', '') for l in f), delimiter='|', quotechar=None)
...
class Catasto(object):
    ...
    def _loadSOGData(self, zipInfoFile):
        ...
        #PersonaFisica.objects.bulk_create(self.persona_fisica_bulk_data)
        PersonaFisica.cadastre_objects.bulk_create(self.persona_fisica_bulk_data, batch_size=1)

        print ("SAVE Persona Giuridica")
        #PersonaGiuridica.objects.bulk_create(self.persona_giuridica_bulk_data)
        PersonaGiuridica.cadastre_objects.bulk_create(self.persona_giuridica_bulk_data, batch_size=1)
...
class CatastoFAB(Catasto):
    ...
    def _loadFABData(self, zipInfoFile):
        ...
        IdentificativiImmobiliari.cadastre_objects.bulk_create(self.identificativi_immobiliari_bulk_data, batch_size=1)
...
class CatastoTER(Catasto):
    ...
    def _loadTERData(self, zipInfoFile):
        ...
        Particella.cadastre_objects.bulk_create(self.particella_bulk_data, batch_size=1)
